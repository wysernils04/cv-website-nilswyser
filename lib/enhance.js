...
(function () {
  ...
  /* ---- Liquid type: the name responds to the pointer ---------------------
     ...
     made touchable. Fine pointers only; per-letter font-variation overrides
     the inherited axis values and restores cleanly on leave. */
  if (hero && name && !reduce.matches && matchMedia('(pointer: fine)').matches) {
    ...
    var liquid = function () {
      ...
      letters.forEach(function (el) {
        ...
        var t = Math.max(0, 1 - d / 280);
        el.style.fontVariationSettings = t > 0.01
          ? '"wght" ' + Math.round(700 + 210 * t) + ', "wdth" ' + Math.round(125 + 15 * t)
          : '';
      });
    };
    var queueLiquid = function () {
      if (!liquidRaf) liquidRaf = requestAnimationFrame(liquid);
    };
