...
(function () {
  ...
  var measureKinetic = function () {
    ...
    if (slot) {
      ...
      // measure the name at its REST axis state, then restore. The !important
      // override outranks the entrance keyframe animation, which would
      // otherwise leak its mid-flight width into the measurement.
      ...
      // pin the slot to the natural height so the fixed phase can't collapse it
      var restH = name.offsetHeight;
      slot.style.height = restH + 'px';
      // If the name rests on two lines, the entrance width-settle would cross
      // the wrap threshold mid-animation — a visible line snap and a real
      // layout-shift (CLS) hit. Skip the settle there; the letter rise still
      // plays, and the width axis still performs in scroll + hover.
      name.style.animationName = restH > lineH * 1.5 ? 'none' : '';
      name.style.removeProperty('font-variation-settings');
      name.style.fontVariationSettings = prevFvs;
      ...
      if (hero && name) {
        ...
        if (slot) {
          ...
          if (nameFixed && dividerDoc) {
            // Dissolve: as the divider approaches the name, shrink + fade.
            // The window is a pure function of y over a STABLE cached basis —
            // never of live layout, which would make the state path-dependent.
            var dView = dividerDoc - y; // divider's viewport y
            // the dissolve COMPLETES while the divider is still well below
            // the name — even a fast wheel tick (~90px/frame) can't produce
            // contact between the line and a visible name
            var fEnd = PIN_TOP + lineH + 70;
            var fStart = fEnd + 260;
